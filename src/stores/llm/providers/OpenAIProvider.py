...
class OpenAIProvider(LLMInterface):
    ...
    def generate_text(self, prompt: str, chat_history: list=[],
        ...
        if not self.generation_model_id:
            self.logger.error("Generation model for OpenA was NOT set")
            return None
    ...
    def embed_text(self, text: str, document_type: str=None):
        ...
        if not self.embedding_model_id:
            self.logger.error("Embedding model for OpenA was NOT set")
            return None
        ...
        if not response or not response.data or not response.data[0].embedding or len(response.data)==0:
    ...
    async def generate_structured_response(self, system_prompt: str, user_prompt: str, response_schema: dict, temperature: float = None):
        if not self.client or not self.generation_model_id:
            self.logger.error("OpenAIProvider: client or model not initialized")
            return None
        ...
        try:
            ...
            self.logger.error(f"OpenAIProvider structured generation error: {e}")
            return None